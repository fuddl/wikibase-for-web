export default ({ vars, manager }) => {
	...
	if (vars.precision > 10) {
		...
		vars.localised = browser.i18n.getMessage('date_month', [
			monthName,
			monthNumber,
			year,
		]);
		...
		vars.localised = browser.i18n.getMessage('date_decade', [
			decadeOrdinal,
			decade,
		]);
		...
		vars.localised = browser.i18n.getMessage('date_cenury', [
			centuryOrdinal,
			century,
		]);
		...
		vars.localised = browser.i18n.getMessage('date_millenium', [
			milleniaOrdinal,
			millenia,
		]);
	} else if (vars.precision < 6) {
		...
		vars.localised = browser.i18n.getMessage('date_year_range', [
			year,
			lowerBound,
			upperBound,
			deltaFormated,
