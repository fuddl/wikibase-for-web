...
import defaultWikibases from '../wikibases.mjs'; // Import your default Wikibases data
...
function Options() {
	...
	useEffect(() => {
		...
		const loadWikibases = async () => {
			...
			// Load the experimental settings from sync storage
	...
	const handleAddInstance = async () => {
		if (newInstance.instance) {
			...
			setInstances({ ...defaultWikibases, ...customWikibasesToStore });
	...
	return html`
		<div>
			...
			`}
			${!showForm &&
			html`<button onClick=${() => setShowForm(true)}>
				Add New Instance
			</button>`}
		</div>
	`;
